...
class CustomApplications(Applications):
	def app_default(self, c):
		...
		if f.extension is not None:
			...
			if f.extension == 'm3u':
				c.flags += 'd'
				return self.either(c, 'gnome-mplayer')
			if f.extension == 'm4v':
				c.flags += 'd'
			...
			if f.extension in ('html', 'htm', 'xhtml'):
				return self.either(c, 'firefox', 'luakit', 'jumanji',
						'luakit', 'elinks', 'lynx')
			if f.extension == 'swf':
				return self.either(c, 'firefox', 'luakit', 'jumanji', 'luakit')
		...
		if f.video or f.audio:
			if f.video:
				c.flags += 'd'
			return self.either(c, 'mplayer', 'gnome-mplayer', 'vlc')

		if f.image:
			if c.mode in (11, 12, 13, 14):
				return self.either(c, 'set_bg_with_feh')
			else:
				return self.either(c, 'gpicview', 'feh', 'eog', 'mirage')
...
CustomApplications.generic('uzbl', 'firefox', 'gpicview', 'evince',
		'gnome-mplayer', 'gimp', 'mirage', 'eog', 'mplayer',
			flags='d', deps=['X'])
